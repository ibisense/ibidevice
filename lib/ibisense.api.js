...
var ibisense = (function ($) {
    ...
    api = {
        ...
        activation: {
            ...
            activateUnregistered: function(options, success, error, always) {
                if (!options.serial) throw "serial number is missing";
                if (!options.secret) options.secret = "";
                var query = {serial: options.serial, secret: options.secret};
                httprequest({
                    type:    "GET",
                    data:    query,
                    url:     ApiBaseURL + "get/activation/activate/",
                    final:   always,
                    success: function (jsonObj, status) {
                        if (success) success(jsonObj.api_key, jsonObj.SUID, status);
                    },
                    error:   function (status) {
                        if (error) error(status);
                    }
            ...
            activateRegistered: function(serial, code, success, error, always) {
                if (!serial) throw "serial number is missing";
                if (!code) throw "activation code is missing";

                var query = {serial: serial, code: code};
            ...
            deactivate: function(serial, code, success, error, always) {
                if (!serial) throw "serial number is missing";
                if (!code) throw "activation code is missing";
                ...
                httprequest({
                    type:    "GET",
                    data:    query,
                    url:     ApiBaseURL + "get/" + ApiKey + "/activation/deactivate/",
                    final:   always,
                    success: function (jsonObj, status) {
                        if (success) success(status);
                    },
